-- CDE Costasur: Revisión del Directorio como etapa explícita.
-- Adición controlada: conserva la firma, permisos y transiciones existentes;
-- solo inserta la etapa directorio entre anteproyecto y planos_tecnicos.
...
begin
  ...
  if p_workflow_stage not in ('autorizacion','anteproyecto','directorio','planos_tecnicos','legal','inicio_obra') then raise exception 'Invalid workflow stage'; end if;
  ...
  if p_workflow_stage in ('anteproyecto', 'directorio') and document_row.category not in ('anteproyecto','planta_conjunto','planta_nivel','elevaciones','secciones','curvas_nivel','memoria_descriptiva','anexos') then raise exception 'Director review requires an anteproject package document'; end if;
  ...
  if not reviewer_allowed then
    select exists (
      ...
      where pm.project_id = p_project_id
        ...
        and (
          (p_workflow_stage in ('autorizacion','anteproyecto','directorio','planos_tecnicos') and ur.role_key = 'revision_tecnica' and ur.department_id = pm.department_id)
  ...
  where d.slug = case when p_workflow_stage in ('autorizacion','anteproyecto','directorio','planos_tecnicos') then 'arquitectura' when p_workflow_stage = 'legal' then 'legal' else 'control_obras' end
  ...
  if p_decision = 'aprobado' then
    if p_workflow_stage = 'autorizacion' then
      ...
      new_phase := 'directorio';
      new_status := 'en_revision';
    elsif p_workflow_stage = 'directorio' then
