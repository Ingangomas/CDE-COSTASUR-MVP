...
const ANTEPROJECT_CATEGORIES = ["curvas_nivel", "planta_conjunto", "planta_nivel", "sotano", "elevaciones", "secciones", "anteproyecto"];
...
export function PlanSetViewer({ projectId, documents, technicalEnabled = true, onUploaded }: { projectId: string; documents: DocumentRecord[]; technicalEnabled?: boolean; onUploaded?: (document?: DocumentRecord) => void }) {
  ...
  const plans = useMemo(() => sortPlans(documents.filter((document) => visibleCategories.includes(document.category))), [documents, visibleCategories]);
  ...
  const uploadCategories = visibleCategories.filter((category) => PLAN_CATEGORY_META[category]).map((category) => ({ value: category, label: PLAN_CATEGORY_META[category].label }));
    ...
    <section className="overflow-hidden rounded-[28px] border border-outline-variant/30 bg-[#eef0f2] shadow-sm">
      ...
      <div className="grid min-h-[680px] grid-cols-1 lg:grid-cols-[270px_minmax(0,1fr)_250px]">
        <aside className="border-b border-outline-variant/30 bg-[#f8f9fa] lg:border-b-0 lg:border-r">
          ...
          {canUpload && isUploadOpen && <div className="border-b border-outline-variant/30 bg-white p-3"><DocumentUpload projectId={projectId} defaultCategory={activeView === "anteproyecto" ? "planta_conjunto" : "arquitectonico"} categories={uploadCategories} titleLabel="Nombre de la hoja" accept=".pdf,.dwg,.dxf" visibleToOwner onUploaded={(document) => { setIsUploadOpen(false); if (document) setSelectedDocumentId(document.id); onUploaded?.(document); }} /></div>}
