...
export function DocumentUpload({ projectId, onUploaded, defaultCategory = "anteproyecto", categories = DEFAULT_CATEGORIES, titleLabel = "Título del documento", accept = ".pdf,.dwg,.dxf,.doc,.docx,image/png,image/jpeg" }: DocumentUploadProps) {
  ...
  const submit = async (event: FormEvent) => {
    ...
    try {
      ...
      const { data: version, error: versionError } = await client.from("document_versions").insert({ document_id: document.id, version_number: 1, original_filename: file.name, storage_path: storagePath, mime_type: file.type || "application/octet-stream", file_size_bytes: file.size, cde_state: "wip", uploaded_by: profile.id }).select("id").single();
