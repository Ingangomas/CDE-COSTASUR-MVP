...
export function ArchitectPortal() {
  ...
  if (!projectId) {
    ...
    const projectCard = (project: ProjectRecord) => <button type="button" key={project.id} onClick={() => void loadWorkspace(project.id)} className="group flex w-full items-center justify-between gap-5 rounded-3xl border border-outline-variant/30 bg-white p-6 text-left shadow-sm transition-all hover:-translate-y-0.5 hover:border-primary/40 hover:shadow-md"><div className="min-w-0"><p className="text-xs uppercase tracking-[0.18em] text-secondary">{project.project_code}</p><h2 className="mt-2 truncate text-xl font-bold text-on-surface">{project.title}</h2><p className="mt-2 text-sm text-secondary">{phaseLabels[project.phase] ?? project.phase.replaceAll("_", " ")}</p></div><span className="flex shrink-0 items-center gap-2 text-sm font-semibold text-primary">Entrar al proyecto <span className="material-symbols-outlined transition-transform group-hover:translate-x-1">arrow_forward</span></span></button>;
