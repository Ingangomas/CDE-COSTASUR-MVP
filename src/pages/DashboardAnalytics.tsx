    ...
    <div className="min-h-full flex-1 overflow-y-auto bg-surface-container-low p-4 pt-8 md:p-10">
      <div className="mx-auto max-w-[1300px] space-y-8">
        ...
        {!loading && !error && <>
          ...
          <section className="rounded-3xl border border-outline-variant/30 bg-surface-container-lowest p-6 md:p-8"><div className="flex items-center justify-between gap-4"><div><p className="text-xs font-semibold uppercase tracking-[0.18em] text-secondary">Operación persistente</p><h3 className="mt-2 text-xl font-bold text-primary">Expedientes asignados</h3></div><span className="text-xs text-secondary">{summary.finalised} finalizados</span></div><div className="mt-5 space-y-3">{projects.slice(0, 8).map((project) => <Link key={project.id} to={role === "admin" ? `/admin/proyectos/${project.id}` : `/${role}/mis-proyectos`} className="block rounded-2xl border border-outline-variant/20 bg-surface-container-low p-4 transition-colors hover:border-primary/40"><div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><div className="min-w-0"><p className="truncate font-semibold text-on-surface">{project.title}</p><p className="mt-1 text-xs text-secondary">{project.project_code} · {project.phase.replaceAll("_", " ")}</p></div><div className="flex items-center gap-4 sm:justify-end"><span className="text-xs font-semibold text-primary">{statusLabels[project.operational_status] ?? project.operational_status}</span><div className="w-24"><div className="h-1.5 overflow-hidden rounded-full bg-surface-variant"><div className="h-full rounded-full bg-primary" style={{ width: `${project.progress_percent}%` }} /></div><p className="mt-1 text-right text-[10px] text-secondary">{project.progress_percent}% físico</p></div></div></div></Link>)}{!projects.length && <p className="rounded-xl bg-surface-container-low p-5 text-sm text-secondary">No hay expedientes asignados para este usuario.</p>}</div></section>
        </>}
      </div>
    </div>
  );
